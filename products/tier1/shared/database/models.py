...
class User(Base):
    ...
    senha_hash = Column(String(255), nullable=True)  # Hash da senha para login
    autenticado = Column(Boolean, default=False)  # Status de autenticação atual
    ultimo_login = Column(DateTime, nullable=True)  # Data do último login
    # Preferências de alertas
    alerta_canal = Column(String(50), default="telegram")  # 'telegram', 'email', 'ambos'
    alerta_intervalo_dias = Column(Integer, default=3)  # 1, 3, 7 ou customizado
    alerta_ativo = Column(Boolean, default=True)
    alerta_horario = Column(String(10), nullable=True)  # 'HH:MM' formato
